...
class Ancestor {
	...
	public function getConfig () {
		if (!$this->config) {
			if (file_exists(app_path() . '/Configs/Cms.php')) {
				$config = new \App\Configs\Cms;
			} else {
				$config = new Config;
			}
			$this->config = $config;
	...
	public function getTemplate ($template = false) {
		...
		if ($this->templateDirectory !== null) {
			...
			$pathToTemplate = base_path() . '/resources/views/cms/' . $this->getType() . '/' . $template;
			if (!file_exists($pathToTemplate)) {
				# 5th - if it's not in <app>/<type>/ check out <app>/
				$pathToTemplate = base_path() . '/resources/views/cms/' . $template;
			}
